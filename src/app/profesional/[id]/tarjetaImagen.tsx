...
/**
 * Genera la tarjeta profesional (foto, oficio, rating, trabajos y QR reales).
 *
 * Se usa tanto para la tarjeta descargable (`tarjeta/route.tsx`) como para
 * la imagen de Open Graph (`opengraph-image.tsx`): es la misma imagen en
 * los dos lugares a propósito — WhatsApp, Facebook, Twitter y LinkedIn leen
 * la misma etiqueta og:image, así que todas terminan mostrando esta tarjeta.
 */
export async function buildTarjetaImageResponse({
  pro,
  host,
  perfilUrl,
}: {
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', fontSize: 16, color: 'rgba(255,255,255,0.85)' }}>
            ...
            <span style={{ display: 'flex', fontWeight: 700, color: '#fff', fontSize: 18 }}>{host}</span>
          </div>
        </div>
      </div>
    ),
