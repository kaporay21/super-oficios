...
import { 
  ...
  UploadCloud, X, Smartphone, Plus, Trash2, FileCheck, Key
} from 'lucide-react';
...
export default function PerfilProfesionalPage() {
  ...
  // Manejo de Especialidades (Ahora desde lista precargada)
  const agregarEspecialidad = () => {
    if (nuevaEspecialidad !== '') {
      setPerfil({ ...perfil, especialidades: [...perfil.especialidades, nuevaEspecialidad] });
  ...
  const eliminarEspecialidad = (index: number) => {
    ...
    setPerfil({ ...perfil, especialidades: actualizadas });
  };

  // Simulación de subida de archivos (Perfil y Certificados)
    ...
    if (e.target.files && e.target.files.length > 0) {
      ...
      setTimeout(() => {
        ...
        if (tipo === 'avatar') {
          ...
          setPerfil({ ...perfil, certificados: perfil.certificados + 1 });
  ...
  const handleSubirDNI = (e: React.FormEvent) => {
    ...
    // Simulamos el envío a Supabase Storage
    setPerfil({ ...perfil, estadoDNI: 'En Revisión' });
    setModalDNI(false);
  ...
  return (
    <div className="bg-[#f7fafc] text-[#181c1e] min-h-screen font-sans selection:bg-[#0f4c81] selection:text-white md:pl-20 pb-24 md:pb-0">
      ...
      <header className="fixed top-0 left-0 w-full z-40 flex items-center justify-between px-4 h-16 bg-white shadow-sm border-b border-gray-200">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="w-8 h-8 rounded-full bg-[#d2e4ff] flex items-center justify-center text-[#00355f] font-bold text-sm border border-gray-200">
            JP
          </div>
        </div>
      </header>
      ...
      <nav className="hidden md:flex fixed left-0 top-16 bottom-0 w-20 bg-white border-r border-gray-200 z-30 flex-col items-center py-8 gap-6">
        ...
        <div className="mt-auto">
          <button className="w-12 h-12 bg-blue-50 text-[#00355f] rounded-xl flex items-center justify-center shadow-sm"><User className="w-6 h-6 fill-current" /></button>
        </div>
      </nav>
      ...
      <main className="pt-24 pb-8 max-w-2xl mx-auto px-4">
        ...
        <div className="space-y-6 relative z-0">
          ...
          <SectionCard title="Mi Oficio" icon={Wrench}>
            ...
            {isEditing ? (
              <button onClick={() => certificadoRef.current?.click()} className="w-full flex items-center justify-between px-4 py-4 hover:bg-gray-50 transition-colors text-left group border-b border-gray-100">
                <div className="flex flex-col pr-4">
                  <span className="text-sm font-bold text-[#00355f]">Certificaciones y Títulos</span>
                  <span className="text-sm text-[#fc8127] font-medium">Toca para subir un nuevo archivo (PDF/Imagen)</span>
                </div>
                <UploadCloud className="w-5 h-5 text-gray-400 group-hover:text-[#00355f]" />
              </button>
            ) : (
              <ListItem title="Certificaciones y Títulos" subtitle={`${perfil.certificados} archivos subidos`} />
            )}
