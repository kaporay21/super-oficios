...
export default function LoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Simulamos un retraso de verificación en la base de datos
    setTimeout(() => {
      setIsLoading(false);
      setSuccessMessage(true);
      
      // Redirección Inteligente Simulada
      setTimeout(() => {
        const mailMinusculas = email.toLowerCase();
        // Si el correo tiene la palabra "pro" o "juan", va al panel profesional
        if (mailMinusculas.includes('pro') || mailMinusculas.includes('juan')) {
          router.push('/panel-profesional');
        } else {
          // Cualquier otro correo (ej: cliente@gmail.com) va al inicio del cliente
          router.push('/');
        }
      }, 1000);
    }, 1500);
